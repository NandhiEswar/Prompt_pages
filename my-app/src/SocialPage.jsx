...
const SocialPage = ({ selectedId }) => {
  ...
  useEffect(() => {
    if (selectedId) {
      const element = document.getElementById(`social-${selectedId}`);
      if (element) {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors">
        ...
        <p className="text-gray-600 dark:text-gray-300">
          Viral hooks, engaging captions, and scripts for all your social platforms.
        </p>
      </div>
            ...
            <div className="p-4 border-b border-gray-100 dark:border-gray-700">
              <div className="flex justify-between items-start mb-3">
                <span className={`px-2 py-1 text-xs font-bold uppercase rounded tracking-wide ${
                  item.category === 'LinkedIn' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                  item.category === 'Twitter' ? 'bg-sky-50 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300' :
                  item.category === 'Instagram' ? 'bg-pink-50 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300' :
